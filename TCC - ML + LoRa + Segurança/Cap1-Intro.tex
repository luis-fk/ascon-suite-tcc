...
% (Observe no parágrafo anterior como são feitas as citações! As referências estão no arquivo abntex2-
% modelo-references.bib, seguindo um formato específico para as informações necessárias. Ah, se você 
% quiser fazer uma citção com o nome do autor no meio do texto, por exemplo dizendo que o 
%\citeonline{abntex2classe} fez um trabalho importante, você usa o citeonline).

% Os capítulo, seções e conteúdos aqui propostos são baseados no documento \textit{Sumário para 
% Monografia de Projeto de Formatura}.
