...
\section{Motivação}
A necessidade de operar com baixo consumo energético eleva a complexidade do uso de dispositivos IoT, uma vez que tentativas de mitigar esse problema podem resultar em simplificações que geram problemas de segurança \cite{nistir8114} \cite{hessel2023lorawan}. Torna-se pertinente, portanto, compreender a natureza desses ataques e suas formas de mitigação, além de estudar a eficiência e a eficácia de modelos de Machine Learning na detecção de intrusões e anomalias.
...
apenas em regras fixas ou limiares estáticos (ex: ``se RSSI < -120 dBm, alertar'') tendem a ser inadequadas, pois o
comportamento normal varia conforme o contexto operacional (dispositivo,
gateway, ambiente) e evolui ao longo do tempo, e métodos
tradicionais têm dificuldade em capturar estruturas complexas em dados
sequenciais, como degradação gradual de equipamento ou padrões não
lineares de transmissão. Abordagens baseadas em aprendizado, ao contrário,
modelam relações temporais, identificam desvios sutis e adaptam-se
dinamicamente à evolução do comportamento normal \cite{chalapathy2019}. A modelagem parte, assim, do comportamento esperado do dispositivo em operação normal, aprendendo padrões legítimos a partir dos dados observados; desvios suficientemente relevantes em relação ao comportamento aprendido são tratados como indícios de anomalia, em uma abordagem adaptativa que se ajusta ao perfil de cada nó monitorado \cite{hayashi2022handsfree}. Neste trabalho, \textit{Machine Learning} é tratado como um dos núcleos centrais da investigação: seu papel não é apenas classificar dados, mas aprender perfis de operação, comparar abordagens de detecção e oferecer uma base experimental para uma etapa posterior de avaliação em borda, permitindo estudar simultaneamente eficácia de detecção, robustez metodológica e viabilidade de implantação em borda.

O padrão Ascon foi selecionado como a primeira escolha como novo padrão da competição de criptografia leve do NIST \citeonline{nist_lwc_finalists} assim como a primeira escolha de criptografia leve da competição CAESAR \cite{caesar_submissions}. A relevância desse padrão se torna clara quando analisado seu baixo custo energético, rapidez, e confiabilidade, garantindo segurança para dados AEAD e ataques de canal colateral \cite{ascon_official}.
...
O Capítulo \ref{cap:metodo} descreve o método do trabalho. Apresenta-se a
